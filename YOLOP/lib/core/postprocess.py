...
def build_targets(cfg, predictions, targets, model):
    ...
    for i in range(det.nl):
        ...
        print(f"gj type: {type(gj)}, gj: {gj}")
        print(f"gain type: {type(gain)}, gain: {gain}")

        # ก่อนบรรทัดที่มี indices.append(...)
        # แปลง gain ให้เป็น tensor หากยังไม่ได้เป็น
        if not isinstance(gain, torch.Tensor):
            gain = torch.tensor(gain, dtype=torch.float32, device=gj.device)

        # ดำเนินการ clamp และแปลงประเภทข้อมูล
        gj = gj.clamp(min=0, max=gain[3] - 1).to(torch.long)
        gi = gi.clamp(min=0, max=gain[2] - 1).to(torch.long)

        # gi = gi.clamp_(0, gain[2] - 1).long()  # แปลง gi เป็น long int


        # Append
        a = t[:, 6].long()  # anchor indices
        indices.append((b, a, gj, gi))  # เพิ่ม indices ที่แปลงแล้ว
