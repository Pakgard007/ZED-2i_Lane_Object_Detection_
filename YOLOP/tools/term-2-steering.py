...
def calculate_turn_direction(da_seg_mask):
    ...
    left_area = np.sum(da_seg_mask[:, :mid_x])  # พื้นที่ด้านซ้าย
    right_area = np.sum(da_seg_mask[:, mid_x:])  # พื้นที่ด้านขวา

    if left_area > right_area:
        return "LEFT", -15  # เลี้ยวซ้าย พร้อมมุมเลี้ยว -15 องศา
    elif right_area > left_area:
        return "RIGHT", 15  # เลี้ยวขวา พร้อมมุมเลี้ยว 15 องศา
    else:
        return "STRAIGHT", 0  # วิ่งตรง ไม่มีการเลี้ยว
...
def detect(cfg, opt):
    ...
    for i, (path, img, img_det, vid_cap, shapes) in tqdm(enumerate(dataset), total=len(dataset)):
        ...
        # คำนวณทิศทางการเลี้ยวจากพื้นที่ขับขี่
        direction, steer_angle = calculate_turn_direction(da_seg_mask)
