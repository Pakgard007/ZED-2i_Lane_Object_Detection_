...
def get_bird_eye_view(image, src_points, dst_points, output_size):
    """
    แปลงภาพเป็น Bird's Eye View (แนวดิ่งเต็มเฟรม ไม่หมุน)
    """
...
def detect(cfg, opt):
    ...
    for i, (path, img, img_det, _, shapes) in tqdm(enumerate(dataset), total=len(dataset)):
        ...
        # ✅ ปรับจุด Perspective Transform ให้ BEV อยู่ตรงกลางแนวดิ่งเต็มเฟรม
        ...
        dst_points = np.float32([
            [400, 1080],  # ซ้ายล่าง
            [800, 1080],  # ขวาล่าง
            [500, 0],     # ซ้ายบน (ปรับให้อยู่กลางมากขึ้น)
            [700, 0]      # ขวาบน (ปรับให้อยู่กลางมากขึ้น)
        ])

        output_size = (1550, 1080)  # ✅ ขยายเฟรมให้เต็มแนวดิ่ง
        ...
        # ✅ ปรับสี พื้นที่ขับขี่ = สีเขียว และ เลน = สีแดง
        ...
        overlay[lane_line_bev == 255] = [0, 0, 255]  # Red for lane lines

        combined_bev = cv2.addWeighted(drivable_bev_colored, 1, overlay, 0.7, 0)

        # ✅ แสดง BEV แนวตั้งเต็มเฟรม
