...
def calculate_turn_direction(da_seg_mask, ll_seg_mask):
    """
    คำนวณทิศทางการเลี้ยวโดยผสมข้อมูลจากพื้นที่ขับขี่และเส้นเลน
    """
    height, width = da_seg_mask.shape
    mid_x = width // 2

    # ตรวจสอบว่าด้านหน้ามีพื้นที่ขับขี่อยู่หรือไม่
    top_area = np.sum(da_seg_mask[:height // 3, :])
    if top_area > 0.5 * np.sum(da_seg_mask):
        return "STRAIGHT", 0

    # คำนวณพื้นที่ซ้ายและขวาของพื้นที่ขับขี่
    left_area = np.sum(da_seg_mask[:, :mid_x])
    right_area = np.sum(da_seg_mask[:, mid_x:])

    # คำนวณตำแหน่งศูนย์กลางของเส้นเลน
    ...
    # ผสมข้อมูลจากพื้นที่ขับขี่และเส้นเลน
    combined_center = (lane_center + (mid_x + (right_area - left_area) / max(left_area + right_area, 1) * mid_x)) // 2
    steer_angle = (combined_center - mid_x) / mid_x * 45  # แปลงค่าเป็นองศา (สมมติขอบเขต ±45)
...
def detect(cfg, opt):
    ...
    for i, (path, img, img_det, vid_cap, shapes) in tqdm(enumerate(dataset), total=len(dataset)):
        ...
        # คำนวณทิศทางการเลี้ยวจากข้อมูลพื้นที่ขับขี่และเส้นเลน
        direction, steer_angle = calculate_turn_direction(da_seg_mask, ll_seg_mask)
