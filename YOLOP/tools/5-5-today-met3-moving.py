...
def get_lookahead_point_from_da_mask(car_position, da_seg_bev_mask, lookahead_distance):
    """
    ค้นหา Lookahead Point จาก Drivable Area Mask ใน BEV ด้วยวิธีประสิทธิภาพสูง
    """
    ys, xs = np.where(da_seg_bev_mask > 0)  # พิกัดทั้งหมดที่เป็นพื้นที่ขับขี่
    if len(xs) == 0:
        return None  # ไม่มีพื้นที่ขับขี่เลย

    points = np.vstack((xs, ys)).T  # shape (N,2)
    distances = np.linalg.norm(points - car_position, axis=1)
    
    # หาจุดที่ระยะทางมากกว่าหรือเท่ากับ lookahead_distance
    valid_points = points[distances >= lookahead_distance]
    if len(valid_points) == 0:
        return None

    # เลือกจุดที่อยู่ใกล้ car_position มากที่สุดใน valid_points (อาจเลือกจุดแรกก็ได้)
    lookahead_point = valid_points[0]

    return tuple(lookahead_point.astype(int))

# ฟังก์ชันแปลงจุดจาก Front View ไป BEV ด้วย Homography Matrix
def transform_point_front_to_bev(point_front, H):
    pt = np.array([point_front[0], point_front[1], 1]).reshape(3, 1)
    pt_bev = H @ pt
    pt_bev /= pt_bev[2, 0]
    x_bev, y_bev = int(pt_bev[0, 0]), int(pt_bev[1, 0])
    return x_bev, y_bev

# ฟังก์ชันสร้าง mask วัตถุใน BEV จากการตรวจจับใน Front View
def create_obstacle_mask_in_bev(bev_shape, detections, H):
    ...
    for det in detections:
        ...
        cx_bev, cy_bev = transform_point_front_to_bev((cx, cy), H)
        # วาดวงกลม buffer รอบวัตถุใน BEV (ขนาด radius ปรับได้)
        cv2.circle(mask, (cx_bev, cy_bev), radius=40, color=255, thickness=-1)
    return mask

def create_obstacle_mask_in_bev(bev_shape, detections, H):
    obstacle_mask = np.zeros(bev_shape, dtype=np.uint8)
    ...
    for det in detections:
        ...
        pts_bev = cv2.perspectiveTransform(pts, H)
        pts_bev = pts_bev.astype(np.int32)

        cv2.fillPoly(obstacle_mask, [pts_bev], 255)
...
while True:
    if zed.grab() == sl.ERROR_CODE.SUCCESS:
        ...
        # สร้าง mask วัตถุใน BEV จากการตรวจจับใน Front View
        # สร้าง mask วัตถุใน BEV
        obstacle_mask_bev = create_obstacle_mask_in_bev(da_seg_bev_mask.shape, det_pred[0], H)

        # ลบพื้นที่วัตถุออกจากพื้นที่ขับขี่ใน BEV
        safe_drivable_area = cv2.bitwise_and(da_seg_bev_mask, cv2.bitwise_not(obstacle_mask_bev))

        # ใช้ safe_drivable_area ในการหาจุด Lookahead
        lookahead_point = get_lookahead_point_from_da_mask(car_position, safe_drivable_area, lookahead_distance)

        # (ส่วนที่เหลือใช้ lookahead_point คำนวณ steering ตามเดิม)

        if lookahead_point is None:
            print("⚠️ No lookahead point found, skipping steering calculation.")
            steering_command = 0
        else:
            ...
            cv2.circle(bev_image, lookahead_point, 10, (255, 0, 0), -1)
            cv2.putText(bev_image, 'Lookahead Point', (lookahead_point[0] - 100, lookahead_point[1] - 15),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 0), 2)

            cv2.line(bev_image, tuple(car_position), lookahead_point, (255, 255, 0), 2)

        cv2.imshow("Obstacle Mask BEV", obstacle_mask_bev)
        cv2.imshow("Safe Drivable Area", safe_drivable_area)
