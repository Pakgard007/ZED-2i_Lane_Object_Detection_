...
def draw_distance_line_in_bev(bev_image, left_lane_x, distance_to_lane, pixels_per_meter):
    """ วาดเส้นระยะห่างจากขอบเลนใน BEV """
    car_x = bev_image.shape[1] // 2  # ตำแหน่งรถในแนวนอน BEV (กลางภาพ)
    ...
    # วาดเส้น
    cv2.line(bev_image, line_start, line_end, color, thickness)
...
while True:
    if zed.grab() == sl.ERROR_CODE.SUCCESS:
        ...
        bev_image = draw_distance_line_in_bev(bev_image, left_lane_x, distance_to_lane, pixels_per_meter)

        # ----------------- เพิ่มข้อความแสดงสถานะการตรวจจับ -----------------
        bev_image = add_text_to_bev(bev_image, distance_to_lane)
        ...
        front_view_with_distance = add_text_to_front_view(front_view_with_distance, distance_to_lane)
